...
// The code-trace panel: an IDE-styled block rendering the lesson's single chosen language (the path's language,
// selected at creation). The header just labels the language — there is no per-trace language toggle (the whole
// path's code, walkthrough and worked example, is generated in one language).
function CodeTracePanel({
  card,
  highlight,
}: {
  card: { code_snippet?: string; code_language?: string };
  ...
  return (
    <div className="w-full overflow-hidden rounded-2xl border border-[#E2DDEC] bg-white shadow-sm shadow-purple-100/40">
      <div className="flex items-center gap-2 border-b border-[#E8E3EF] bg-[#F6F2FF] px-4 py-2.5">
        ...
        <span className="ml-1 text-sm font-black text-foreground">{LANG_LABELS[lang] || lang}</span>
      </div>
      <CodeWithHighlight
