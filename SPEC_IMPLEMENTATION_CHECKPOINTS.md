...
> concept in any is a one-file edit to `families/*_specs.py`, gate-verified. **~174 adapters (was 30 pre-CP12)**
> spanning physics, algebra, geometry, chemistry, finance, statistics (incl. paired-data: weighted mean,
> covariance), sequence, structures, discrete, calculus, numerical, plus CS. Basic calculus (poly derivative/
> integral), numerical methods (Newton/gradient descent), stateful structures (stack/queue/hash/LRU/set/min-
> stack/counter), and broad math/science/finance formulas are all covered as data. **Autonomous data-authoring
> has reached saturation** of the clean/generically-safe/verifiable concepts. Remaining: (a) the **live-regen
> human sign-off** (needs the product + an API key — the CP7 "feels useful" gate); (b) optional CAS-backed
> symbolic calculus (chain rule, non-polynomial integrals); (c) constraint-heavy or 2-list concepts (trig
> ratios, correlation) that need per-concept guards; (d) Part-C conceptual topics (stay guided, not adapters).
