...
COMMON_RULES = {
    "card_density": [
        ...
        "For an ALGORITHM/iterative process card, keep frames such as Starting state, Repeated action, State update, Stopping condition, and Output rule as main bullets; put variables, actions, conditions, and results as subpoints.",
        "For a MATH FORMULA process card (a one-shot calculation, not a loop), do NOT use the Starting state / Repeated action / Stopping condition framing — use the calculation stages (identify the givens, state the formula, substitute, compute, interpret) as the main bullets instead.",
...
VISUAL_CARD_RULES: dict[str, dict[str, dict[str, Any]]] = {
    ...
    TopicType.ALGORITHM_WALKTHROUGH.value: {
        ...
        "complexity_analysis": {"visual_type": "none", "purpose": "Time/space complexity with the cost driver, plus when to prefer this algorithm and when not — text-only."},
    ...
    TopicType.DATA_STRUCTURE_OPERATION.value: {
        ...
        "complexity_analysis": {"visual_type": "none", "purpose": "Time/space complexity of the operation with the cost driver, plus when it is efficient and when not — text-only."},
    ...
    TopicType.CODING_IMPLEMENTATION.value: {
        ...
        "complexity_analysis": {"visual_type": "none", "purpose": "Time/space complexity of the implementation with the cost driver, plus when to prefer it and when not — text-only."},
...
TOPIC_BLUEPRINTS: dict[str, Blueprint] = {
    ...
    TopicType.ALGORITHM_WALKTHROUGH.value: {
        ...
        "avoid": [
            "Do not include full code before the algorithm behavior has been explained.",
            "Do not include a comparison card — comparing this algorithm with others belongs in a dedicated compare_distinguish topic, not in the walkthrough (avoids redundancy).",
        ],
