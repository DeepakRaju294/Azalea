...
_CODE_GEN_SYSTEM = (
    ...
    "produce THAT. ACCEPT THE INPUT AS A STANDARD BUILT-IN STRUCTURE the caller can pass as plain data "
    "— a list for sequence algorithms; for a graph, an adjacency map `{node: [(neighbor, weight), ...]}` "
    "OR a `(num_vertices, edges)` pair where `edges` is a list of `(u, v, weight)` — NEVER a custom "
    "`class Graph` / `class Node` parameter the caller would have to build first (the entry function "
    "must be runnable on plain data). For a graph algorithm, return the real result the name promises — an MST function "
