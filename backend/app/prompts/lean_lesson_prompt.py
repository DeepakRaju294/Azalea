...
- HELPER FUNCTIONS MUST BE SEPARATE, NOT NESTED: if the implementation uses a recursive helper or any auxiliary function, define it as its own TOP-LEVEL function placed BELOW the main function (one blank line between them), and pass shared state (e.g. the result list) in as a parameter. Do NOT define the helper nested inside the main function (no closure over an outer `result`). This mirrors how solutions are written in coding IDEs/LeetCode (a public method that calls a separate helper) and keeps each function readable on its own. Example shape: `def inorder(root):` builds an empty result, calls `traverse(root, result)`, returns result — then a separate `def traverse(node, result):` does the base-case check, the recursive calls, and the append.
