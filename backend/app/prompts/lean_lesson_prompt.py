...
2. TRAVERSAL STRUCTURE. For tree/graph/BST/traversal/DFS/BFS topics (non-boundary), the VISUAL ANCHOR is one irregular 7-10 node structure chosen before writing any worked_example card. Every worked_example continuation reuses the same nodes, edges, positions, and labels — only the active node, active edge, tracker values, visited/completed states, output, and call stack change between cards.
...
4. CODING WORKED EXAMPLE COMPLETENESS. For coding_implementation worked_example cards, every card carries the complete final code_snippet (identical across the sequence) and uses highlight_lines_per_step to mark the line/block executing at that step. Bullets describe runtime state after that line; do not rebuild the code here.
4a. CODING WORKED EXAMPLE = AN EXECUTION TRACE, NOT A CONCEPT RECAP. A coding_implementation worked_example must walk through how THE CODE runs, line by line — distinct from an algorithm/data-structure worked_example, which narrates the concept. Each step's bullets must name the line being executed and what it concretely does to the running state: which line runs now, whether a condition was evaluated and which branch it took OR did not take (e.g. "the `if node is None` check runs and is false, so we skip the early return and continue"), and the exact change to a variable/accumulator/pointer. EVERY executable line must be highlighted at some step across the trace, including condition checks that evaluate to false and recursive calls that descend before any visit happens — a line that runs but is never highlighted is a gap. Do NOT collapse several lines' effects into one conceptual sentence ("it traverses the left subtree and adds the node"); give each executed line its own step so the highlight advances one line at a time.
4b. CODING WORKED EXAMPLE — SETUP STEPS + MINIMUM LENGTH. The trace MUST OPEN WITH THE SETUP: each variable-initialization line (e.g. `low = 0`, then `high = len(arr) - 1`) is its OWN step, highlighted one at a time, BEFORE the main loop/recursion begins — so the learner sees the starting state being built. Then the loop/recursion runs. Choose an example INPUT large enough that the algorithm performs AT LEAST 4 meaningful steps after setup (e.g. for binary search use a sorted array of >= 7 elements so the loop iterates several times; do not pick an input that finds the answer on the first comparison). A 2-3 step trace is too short — the learner must see the loop repeat.
...
- CRITICAL — code_walkthrough card size: each card should introduce AT MOST 3 new lines of code. Cards with 1-2 new lines are fine. If the next functional block has more than 3 lines, split it across multiple adjacent code_walkthrough continuation cards (e.g. "DFS Part 2: Loop Header", "DFS Part 3: Pop and Check", "DFS Part 4: Push Neighbors") rather than overloading one card. This keeps each card's per-bullet explanation focused and the per-line reveal experience meaningful. Empty lines and pure-syntax lines (lone `}`, bare `else:` when its body is on the next card) do not count toward the 3-line budget.
- For coding_implementation worked_example cards: include code_snippet as the complete implementation and highlight_lines_per_step so each example step highlights the currently executed line or functional block. The i-th range must correspond to the i-th top-level bullet group.
- For code_trace: use the code block as the focus area. Fill code_snippet/code_language and do not duplicate code in visual_description. Do not fill visual_columns or visual_rows for code_trace.
- For step_flow and causal_chain: fill visual_steps with at least 2 steps.
- For every visual_steps item, set kind, step_title, visual_label, and step_detail in addition to label/description. visual_label is for the diagram only: 1-4 words, no trailing punctuation, and not a sliced/truncated explanation. Good visual_label examples: Start, Loop, Pop, Check, Push, Update, Done, Stack Ready, Pop + Explore.
- visual_label values in the same visual must be unique unless the exact same action intentionally repeats. Each visual_label should name that step's distinct role, not the topic's repeated state.
- For step_flow visual_steps, use label and description for every step. When useful, also fill mini_visual with a 2-6 word cue, formula with a compact expression, cases with 2-3 branch labels, and active=true for the current/focal step. Keep labels 1-4 words and descriptions 6-16 words.
- For progressive_step_flow: create exactly one visual_step for each top-level/main bullet in the card points, in the same order. Do not summarize several main bullets into one visual step and do not omit the final main bullet. The visual step label must match the learner-facing main bullet frame, shortened only if needed (for example: "Starting state", "Each iteration", "State update", "Stop when", "Output"; or action labels like "Visit current", "Recurse left" when those are the main bullets). Do NOT break one main bullet into hidden micro-steps such as separate "Pop top", "Visit node", and "Push neighbors" if the text has those actions grouped under one main bullet. description: LEAVE EMPTY ''. ALWAYS fill mini_visual with a separate 3-5 word state/action cue from that same bullet group (for example: "stack is empty", "current = root", "go to node.left", "result += value"). Do not make mini_visual by truncating the bullet sentence; it must read as a complete compact cue. Set active=true only on the step this card is teaching — all others active=false.
- FORBIDDEN for progressive_step_flow labels: never use the card title or the topic name as a step label (e.g. "Preorder Traversal Process Steps" is the card TITLE — it is not a step label). Never use vague placeholders like "Trace The Steps Of", "Step One", "First Step", "Process Step". Never repeat the same label across two steps in the same visual. Every step must have a UNIQUE, CONCRETE verb phrase that names the action taking place.
- For algorithm-specific canonical step labels and mini_visual examples (DFS, BFS, BST traversals, binary search, sliding window, merge sort, quicksort), see the topic-family appendix at the end of the user prompt when one applies. For other topics, pick concrete verb-phrase labels (1-4 words) that name the action taking place — never "Step 1", the card title, or the topic name.
- For comparison_table and state_change: fill visual_columns, visual_rows, and visual_highlight_row when useful. Every row must have exactly the same number of cells as visual_columns.
- For formula_card: fill visual_formula, visual_symbols, and visual_when_to_use.
- For misconception: fill visual_wrong, visual_correct, visual_wrong_label, visual_correct_label, and visual_why.
- For practice_feedback placeholders: fill visual_steps with 2-3 compact steps showing what the learner should predict, check, or compare. Do not invent learner answers before the learner responds.
- For topic_snapshot: fill visual_center (topic name, 1-3 words) and visual_nodes (1-3 nodes MAXIMUM). Each node label must be 1-2 words — a structural or orientation clue, not a sentence. Leave description EMPTY on every node. Purpose: visually answer "what is this?" with the minimum needed to identify the concept.
- For concept_snapshot: fill visual_center (concept or structure name) and visual_nodes (3-5 nodes). Each node label is a key anatomical part name (1-2 words, e.g. 'Header', 'Payload', 'Checksum', 'Pointer', 'Bucket'). Leave description EMPTY. Purpose: show the internal anatomy — what does this look like?
- For edge_case_snapshot: fill visual_center (2-4 words = edge case name, e.g. 'Empty BST') and visual_nodes with 0-2 nodes MAXIMUM. If showing an empty structure, use 0 nodes. If showing a single element, use 1 node. Leave description EMPTY. Purpose: visually anchor the boundary condition — near-empty is correct.
- For relationship_map: fill visual_center (parent concept name) and visual_nodes (3-6 nodes). Each node label is a child term (1-2 words). Each node relation is a 1-word role label (has/is/uses/calls). Leave description EMPTY. Purpose: show term hierarchy and roles without definitions.
- For relationship_map on roadmap cards (blueprint_key='roadmap'): set visual_center exactly to "What you will learn". Fill visual_nodes with one node per study path topic — use the topic name as the label, shortened to 3-5 words if longer. Set relation='' on every node. Do not add descriptions or explanations.
- For node_link_diagram on background cards (blueprint_key='background'): generate a DENSE, COMPLEX visual — 8-10 nodes minimum — showing the complete system filled with realistic simulation data. Node labels must be short data values of at most 3-4 characters: 1-3 digit integers for trees, single or double letters for graphs (A, B, C, D, E, F, G), small integers for linked lists. For BST topics: PICK YOUR OWN unique 1-3 digit integer values for the 7-10 nodes. Different lessons MUST use different value sets — do not repeat the same numbers across examples. The values must satisfy the BST property (every node's left subtree values are smaller, right subtree values are larger) and the tree MUST be asymmetric (at least one internal node has only one child — avoid perfectly balanced/complete trees that look like textbook diagrams). For graph algorithms: show a dense graph with 6-8 lettered nodes and 7-10 edges, weighted only for MST / Dijkstra / Bellman-Ford / shortest-path / A*; unweighted for BFS / DFS / topological sort. NEVER use structural description words as labels (never "Root", "Left Child", "Right Child", "Node", "Leaf", "Parent"). Do not begin a traversal; show the structure at rest. All edges style='solid', no node marked active.
- For node_link_diagram on worked_example, process, or continuation cards: node labels must be short data values of at most 3-4 characters (integers for trees, letters for graphs) — never concept names, structural descriptions, or topic descriptions. Fill visual_nodes and visual_edges so the frontend can render the actual tree/graph. Keep the same full example structure visible across every step; do not shrink the diagram to only the local node and its children. Use 7-10 nodes for tree/graph traversal main examples. Each visual_node needs id, label, relation, description, x, and y. Each visual_edge needs from, to, label, and style. For trees/BSTs, put the root near x=50 y=16 and spread children widely below it. For graph algorithms such as MST, shortest path, DFS, BFS, Prim, Kruskal, or Dijkstra, do not use a root/tree layout; spread nodes around the canvas so edges are visible. Mark the node relevant to the current step by including "current" or "active" in that node's relation or description. For graph algorithms where an edge is being considered/selected, set that edge style="traversal" and include the edge label/weight. Use style="solid" for non-current edges.
- For traversal node_link_diagram visuals, also set explicit states. Node state must be one of unvisited, discovered, newly_discovered, current, completed, skipped. Edge state must be one of unchecked, active, traversed, checked, skipped, completed. Do not use one generic highlight for everything reached. Current node = strongest purple; newly_discovered = gold; discovered/visited = pale lavender; completed = muted lavender with check; unvisited = neutral. Active edge = thick purple; traversed = solid purple; checked/skipped = dashed gray; unchecked = thin gray.
- CRITICAL for ALL node_link_diagram visuals: visual_nodes[i].label = the DATA VALUE stored in that node — a short integer (for trees/linked-lists), single letter (for graphs), or short code. The label must be ONE TOKEN and at most 4 characters. Use "A", not "Node A"; use "42", not "Node 42"; use "R", not "Root". This is NOT a name or structural identifier. The relation field is where the structural role goes ("root", "node", "leaf", "current", "active"). NEVER put structural descriptions like "Node A", "Root", "Left Child", "Right Child", "Leaf Node", "Parent" in the label field. IMPORTANT: set each node's id equal to its label (if label is a number, id is the same number as a string) and use those same data values as edge from/to fields — this keeps node IDs and edge endpoints consistent. Pick varied values per example; do NOT default to the same numbers across different lessons. AVOID the over-used set 50/30/70/20/40/60/80 (and minor permutations) — pick a different value set so the learner doesn't see identical trees across different lessons.
- TREE TRAVERSAL WORKED EXAMPLES — EMIT EXACTLY ONE CARD: for inorder/preorder/postorder/level-order tree (BST/binary-tree) traversals ONLY, Azalea deterministically regenerates the complete step-by-step trace from your worked example. So emit EXACTLY ONE worked_example card — the setup/first step — carrying the COMPLETE node+edge structure and the initial state. Do NOT emit a card per visited node, and do NOT repeat the structure across continuation cards: every extra step card you emit is discarded and regenerated, and emitting the full node/edge list many times wastes the output budget and can truncate the lesson. One worked_example card for the tree traversal is enough. (This overrides the "persistent structure across continuations" rule below FOR TREE TRAVERSAL TOPICS ONLY.) This does NOT apply to GRAPH BFS/DFS traversals or any other topic: graph traversals are NOT regenerated this way, so they MUST follow the persistent-structure rule below and emit the COMPLETE node+edge list on every step card.
- CRITICAL — PERSISTENT STRUCTURE ACROSS TRACE CONTINUATIONS: every worked_example continuation card emits the COMPLETE structural data it received on its first card, not a stripped-down view. This applies uniformly to:
    - tree/BST traversals (every card emits ALL 7-10 node values + complete visual_edges)
    - graph traversals: BFS, DFS, MST, shortest path (every card emits the COMPLETE node + edge list)
    - array algorithms: binary search, sliding window, two pointers (every card emits the FULL visual_array_values; only pointers/ranges/annotations change)
    - linked list operations (every card emits the FULL node chain)
    - DP tables (every card emits the FULL table; only the currently-filled cell changes)
  Only the HIGHLIGHTS, STATE TRACKERS, ACTIVE NODE/INDEX/LINE, and traversal/state metadata change between continuation cards. The base structure stays identical. If you emit empty or stripped visual data on later cards, the trace becomes inconsistent and the renderer cannot show movement within the same diagram. Every single worked_example continuation card repeats the full structure.
- visual_description for ANY node_link_diagram MUST name concrete data values (integers/letters) and the structural shape — it must NOT use words like "concept", "central", "types", "kinds of", "connected to", "represents". If you find yourself writing those words inside a node_link_diagram description, you are generating the wrong kind of diagram — switch to listing actual data values.
- For path_progress: fill visual_steps with the path position, current topic, and next topics. Use topic names as labels only — no descriptions.
- For graph_chart: fill visual_x_label, visual_y_label, visual_data_points, and visual_key_points. Use only meaningful numeric data.
- For array_state_diagram: fill visual_array_values, visual_array_pointers, visual_array_ranges, and visual_array_annotations. Array-algorithm specifics (binary search, sliding window, merge sort, quicksort patterns) are in the topic-family appendix at the end of the user prompt when one applies.
- For circuit_diagram, spatial, and source_annotation (renderer support is partial): use visual_description as a storyboard and leave other structured visual fields null.
- For visual_* and code_* fields that do not apply to the chosen visual_type, return null (the schema permits nullable on every type-specific field). Use null, NOT "" or []. This tells the renderer "this field is intentionally not in use for this card", saves output tokens, and frees attention for the fields you ARE filling. Example: a comparison_table card fills visual_columns/visual_rows/visual_highlight_row and returns null on every other visual_* field including visual_nodes, visual_steps, visual_formula, visual_array_*, etc.
- visual_description is a plain-English scene/state storyboard, naming the before state, the action/change, and the after state. For node_link_diagram, it is a SCENE SUMMARY only — node labels come from the data values in each node, not from visual_description. If a card is a progressive reveal, describe only the new change.
...
CODING WORKED_EXAMPLE — DUAL-PANEL FORMAT (visual + code):
Coding worked_example cards present TWO synchronized panels per step: a DATA-STRUCTURE VISUAL on the left showing the runtime state, and the CODE BLOCK on the right with the line(s) responsible for this step highlighted. Both panels share the same step index.
- Choose the visual_type to match the data structure the code manipulates:
  - Tree / BST / graph algorithms (any recursion over nodes, traversals, search/insert/delete, BFS/DFS, MST, shortest path): visual_type = "node_link_diagram". Fill visual_nodes and visual_edges with the SAME values you use in the trace bullets so the diagram values match the code's runtime state. Mark the current node by including "current" in its relation. For traversal/visit algorithms, fill `traversal_path` with the sequence of nodes visited so far.
  - Array / sorted-array / pointer / window algorithms (binary search, two-pointer, sliding window, merge step, prefix sum): visual_type = "array_state_diagram". Fill visual_array_values with the concrete array, plus visual_array_pointers for the named program variables (low/high/mid/i/j/k/left/right/slow/fast) at their current indices for this step.
  - Other algorithms (DP table fill, hash-map updates, generic state transitions): visual_type = "state_change". Fill visual_columns/visual_rows with the program state at that step.
- The code panel (right side) is driven by code_snippet + highlight_lines_per_step. The i-th range corresponds to the i-th step bullet group; the visual on that same card must show the data-structure state AFTER executing that line/block.
- The code panel always displays the complete implementation. Do not hide future lines, do not use max-line behavior, and do not emit only the current block for coding worked_example.
- Step title format: "Step <N> of <total>: <verb-phrase action>" (e.g. "Step 2 of 7: Go left from 40 to 30", "Step 4 of 9: Compute mid = (low+high)//2"). The first card is step 1 of <total>, the last is <total> of <total>.
- Per-card "what's happening" text (visual_description) is one short sentence stating what THIS line does in plain English, referring to the highlighted code line and the highlighted visual element (e.g. "From 40, recurse left into the subtree rooted at 30 — the call inorder(node.left, result) on line 5 pushes a new frame and moves current to 30").
- The runtime-state bullets are short data facts only. Use `name=value` syntax ONLY for actual Python variables in the code (`result=[20, 30]`, `current=25`, `i=3`, `low=0`). For the implicit call stack in recursive code, write `Call stack: [40 → 30]` in plain English with title-case — NEVER `call_stack=[40, 30]` because no such variable exists in the source. One bullet per tracked item.
- For recursive implementations, list "Call stack: [...]" as a tracked item in Currently/Now bullets (using plain-English title-case format, NOT variable-assignment syntax). Push a frame on every recursive descent, pop on every return. For iterative implementations with an actual `stack` variable in the code, write it as `stack=[40, 30]` since that IS a real variable.
...
- HARD FAILURE: if a tree or graph traversal main worked example has fewer than 7 nodes, regenerate it with an irregular 7-10 node structure.
...
- THE FIRST worked_example card MUST state the scenario in plain English BEFORE the trace begins. Set visual_description on the first card to a sentence naming (a) the algorithm being applied, (b) the input data with the SPECIFIC values you chose for THIS example, and (c) the goal/expected output. PICK YOUR OWN values — do not copy from these templates verbatim, and use different values across different lessons. Template shapes (substitute with your chosen values):
  ...
  - Merge sort: "Sort the array <ARRAY> using merge sort; expected output: sorted ascending."
...
- For graph DFS/BFS worked examples, Azalea may replace your worked_example cards with a deterministic execution trace. If you generate them yourself, follow the same convention: DFS stack top = rightmost, mark on push, neighbors alphabetical, push in reverse-alphabetical; BFS queue front = leftmost, mark on enqueue, enqueue alphabetical.
- Canonical visual step labels: DFS (iterative) → ["Pop top", "Visit node", "Push children"]; BFS / level-order → ["Dequeue front", "Visit node", "Enqueue children"]. Mini_visual examples: "Dequeue front" → "queue.popleft()".
- For node_link_diagram visuals: spread nodes around the canvas so edges are visible — DO NOT use a root/tree layout. Show the graph at rest in background cards (no node marked active); mark the node relevant to the current step by including "current" or "active" in that node's relation or description on worked_example cards.""",
...
- STRUCTURAL TEMPLATE for BST node_link_diagram output (PICK YOUR OWN values — do not copy any specific numbers from this template). For an asymmetric 8-node BST you might generate something like nodes=[{"id":"<R>","label":"<R>","relation":"root","x":50,"y":16},{"id":"<L>","label":"<L>","relation":"node","x":28,"y":42},{"id":"<R2>","label":"<R2>","relation":"node","x":72,"y":42},{"id":"<LL>","label":"<LL>","relation":"leaf","x":15,"y":66},{"id":"<LR>","label":"<LR>","relation":"node","x":40,"y":66},{"id":"<LRX>","label":"<LRX>","relation":"leaf","x":48,"y":85},{"id":"<RL>","label":"<RL>","relation":"leaf","x":62,"y":66},{"id":"<RR>","label":"<RR>","relation":"leaf","x":85,"y":66}], edges=[{"from":"<R>","to":"<L>","label":"","style":"solid"},{"from":"<R>","to":"<R2>","label":"","style":"solid"},{"from":"<L>","to":"<LL>","label":"","style":"solid"},{"from":"<L>","to":"<LR>","label":"","style":"solid"},{"from":"<LR>","to":"<LRX>","label":"","style":"solid"},{"from":"<R2>","to":"<RL>","label":"","style":"solid"},{"from":"<R2>","to":"<RR>","label":"","style":"solid"}]. Substitute <R>, <L>, <LL>... with integer values YOU choose that satisfy BST ordering. Note the asymmetric shape: node <LR> has only one child (<LRX>) — this is REQUIRED, do not produce a perfectly balanced tree. AVOID the over-used set 50/30/70/20/40/60/80 — pick a different value set so the learner doesn't see identical trees across different lessons.
- For BST node_link_diagram on worked_example cards: put the root near x=50 y=16 and spread children widely below it. Mark the node relevant to the current step by including "current" or "active" in that node's relation or description. Every continuation card emits the COMPLETE node + edge list — only the active node and tracker fields change.
- Canonical visual step labels: Preorder → ["Visit current", "Recurse left", "Recurse right"]; Inorder → ["Recurse left", "Visit current", "Recurse right"]; Postorder → ["Recurse left", "Recurse right", "Visit current"]; Level-order / BFS → ["Dequeue front", "Visit node", "Enqueue children"]. Mini_visual examples: "Visit current" → "result += val"; "Recurse left" → "go to node.left"; "Recurse right" → "go to node.right".
...
- For array_state_diagram visuals, fill visual_array_values (visible array/list values as strings, max 16 cells), visual_array_pointers (each item needs label, index, side="top"/"bottom" — labels like left, right, i, j, slow, fast), visual_array_ranges (each item needs label, start, end — use for windows, subarrays, sorted partitions), and visual_array_annotations (compact computed facts such as "sum=21", "need=7", "best=3", "target=17").
- For multi-stage array overviews (merge sort, quicksort partitioning, multi-stage merging), fill visual_array_rows with multiple rows. Each row needs label, values, and emphasis. Use rows to show the whole system: original → split → sorted halves → merged result. For generic divide-and-conquer without a concrete array, use progressive_step_flow instead.
- To show multiple sub-arrays at the same level within one row, insert "|" as a separator value in the values list. Example for merge sort split level: values=["38","27","43","3","|","9","82","10"] renders as two side-by-side array blocks.
- For merge sort background cards, generate 4 rows: (1) label="Original" values=full array emphasis=true, (2) label="Split" values=left half + "|" + right half, (3) label="Sorted halves" values=sorted left + "|" + sorted right, (4) label="Merged" values=final sorted array emphasis=true.
...
- Canonical visual step labels: Binary search → ["Compute mid", "Compare to target", "Shrink range"]; Sliding window → ["Expand right", "Update state", "Shrink left"]; Merge sort merge → ["Compare heads", "Pick smaller", "Advance pointer"]; Quicksort partition → ["Choose pivot", "Partition around", "Recurse halves"]. Mini_visual examples: "Compute mid" → "mid = (l+r)//2".""",

    "coding_implementation": """CODING IMPLEMENTATION RULES (this topic teaches an implementation through code_walkthrough cards):
- ONE STATEMENT PER LINE (house style for this teaching format): write the function body so each executable action is on its own line — the base-case check, each value-collection/visit, each recursive call, each enqueue/dequeue, each pointer update is a separate line. Do NOT collapse the body into a single combined expression (e.g. write the visit and the two recursive calls on three lines, NOT `return f(left) + [val] + f(right)` on one line). This does not change the algorithm or method you choose — it is purely formatting so the code_walkthrough and the worked-example trace can highlight the exact line executing at each step.
- A MAIN FUNCTION PLUS A SEPARATE HELPER (plain top-level functions by default). When the implementation uses recursion or any auxiliary routine, the code_snippet MUST contain BOTH: (a) a MAIN function that is the entry point — it builds the containers/accumulators (e.g. `result = []`), calls the helper, and returns the result — AND (b) a SEPARATE helper function that does the actual work (the base-case check, the recursive calls, the visit/append). NEVER emit only the helper: a snippet that is just `def traverse(node, result):` with no entry point is WRONG — it hides how the function is called and confuses learners. Write BOTH as TOP-LEVEL functions (the helper BELOW the main), SEPARATE THE TWO FUNCTIONS BY TWO BLANK LINES, and pass shared state (the result list) into the helper as a parameter. Do NOT nest the helper inside the main function (no closure over an outer `result`). DO NOT use a `class` or the `self` keyword unless the topic is explicitly object-oriented (e.g. "implement a Stack/HashMap class", an OOP-design topic) — ordinary algorithm/function implementations are plain functions, never methods. Exact shape to follow:
