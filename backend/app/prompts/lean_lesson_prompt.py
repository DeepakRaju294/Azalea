...
- For study_path_introduction, when the intro NAMES the assumed prerequisites (the ideas the path assumes the learner already knows), give each named prerequisite a ONE-LINE plain-language orientation of what it is — roughly 6 to 12 words — not the bare term. This is orientation so a first-time learner can self-check whether they know it, NOT reteaching. Bad: "Voltage". Good: "Voltage — the electrical 'push' that drives current through a circuit." Keep each to a single clause: no second sentence, no formula, no sub-bullets. This applies ONLY to the intro's prerequisite listing; body topics still use assumed prerequisites without definition.
