...
- EXCEPTION — a FORMULA / one-shot CALCULATION process card (applying an equation, e.g. Ohm's law, kinetic energy, compound interest — NOT an iterative algorithm) has no iteration or termination, so do NOT use the "Starting state / Repeated action / State update / Stopping condition / Output rule" framing. Use the calculation stages as the main bullets instead: identify the givens, state the formula (and any precondition), substitute the values, compute, interpret the result. Keep genuinely iterative numerical methods (Newton's method, a series) on the repeated-step framing.
...
- CRITICAL — every practice card (card_type "quick_practice") MUST fill practice_question with the exact, complete, gradeable question text, and practice_answer with the correct answer (a value, expression, or short phrase — not an explanation). These are the fields the app's interactive practice actually runs on; a practice card whose points describe a problem but leaves practice_question/practice_answer null or empty SHIPS WITH NO WORKING PRACTICE, even though the card text looks complete to a reader. points may restate the setup/scenario for readability, but the graded question and its answer belong in practice_question/practice_answer, never only in points. If the question has discrete options, also fill practice_choices with 3-5 plausible choices including the correct one; otherwise leave practice_choices empty.
...
- INTERACTIVE LINKS: do NOT generate any interactive_links yourself — leave `interactive_links` empty. Popup term glosses (action "popup_only") are retired; open_study_path and review_earlier_topic links are added automatically by the system. If a term is hard enough that a beginner would stumble on it, EXPLAIN it in the card text at first use (or move it to a components_terms card per the assumption-ledger rules) — never as a popup gloss.
...
- For study_path_introduction, the prerequisite / foundational-knowledge card lists the ideas the path assumes the learner already knows. Each prerequisite MUST be a concrete NAMED concept followed by a one-line plain-language "what it is" (roughly 6 to 12 words) — the SAME shape as a key-terms entry, so the learner can self-check whether they know it. It is orientation, NOT reteaching.
  - REQUIRED shape: "<Concept name> — <what it is>". Good: "Electric charge — the property of matter that makes current flow." Good: "Trigonometric ratios — sine, cosine, and tangent relating a right triangle's sides."
  - BANNED goal/meta phrasing — never start a prerequisite with "Understanding", "Knowledge of", "Awareness of", "Familiarity with", "Basic understanding of", "An understanding of", "How to", or any verb-first learning-goal wording. Name the concept itself, then explain it. Bad: "Understanding electric charge and its flow through a conductor." Bad: "Knowledge of basic electrical units." Bad: "Awareness of how resistors function."
  - Keep each to a single clause: no second sentence, no formula, no sub-bullets. This applies ONLY to the intro's prerequisite listing; body topics still use assumed prerequisites without definition.
